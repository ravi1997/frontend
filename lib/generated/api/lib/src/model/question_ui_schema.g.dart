// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'question_ui_schema.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

QuestionUISchema _$QuestionUISchemaFromJson(Map<String, dynamic> json) =>
    $checkedCreate(
      'QuestionUISchema',
      json,
      ($checkedConvert) {
        final val = QuestionUISchema(
          id: $checkedConvert('_id', (v) => v),
          createdAt: $checkedConvert('created_at', (v) => v),
          placeholder: $checkedConvert('placeholder', (v) => v),
          style: $checkedConvert('style', (v) => v),
          updatedAt: $checkedConvert('updated_at', (v) => v),
          visibleHeader: $checkedConvert(
            'visible_header',
            (v) => v as bool? ?? false,
          ),
          visibleName: $checkedConvert('visible_name', (v) => v),
        );
        return val;
      },
      fieldKeyMap: const {
        'id': '_id',
        'createdAt': 'created_at',
        'updatedAt': 'updated_at',
        'visibleHeader': 'visible_header',
        'visibleName': 'visible_name',
      },
    );

Map<String, dynamic> _$QuestionUISchemaToJson(QuestionUISchema instance) =>
    <String, dynamic>{
      '_id': ?instance.id,
      'created_at': ?instance.createdAt,
      'placeholder': ?instance.placeholder,
      'style': ?instance.style,
      'updated_at': ?instance.updatedAt,
      'visible_header': ?instance.visibleHeader,
      'visible_name': ?instance.visibleName,
    };
