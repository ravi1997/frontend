...
class _FormAccessSettingsState extends State<FormAccessSettings> {
  ...
  void _emit(AccessPolicy policy) => widget.onChanged(policy);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text('Access / Privacy', style: Theme.of(context).textTheme.titleLarge),
          ...
          child: Column(
            children: [
              RadioListTile<String>(
                ...
                subtitle: const Text('Anyone with the link can open the form.'),
                value: 'public',
              ),
              RadioListTile<String>(
                contentPadding: EdgeInsets.zero,
                title: const Text('Private'),
                subtitle: const Text('Only approved users can open the form.'),
                value: 'private',
              ),
            ],
          ),
        ...
        if (isPassword) ...[
          ...
          PropertyBuilderUtils.buildTextField(
            ...
            onChanged: (val) =>
                _emit(policy.copyWith(passwordHash: val.isEmpty ? null : val)),
          ),
          const SizedBox(height: 12),
          PropertyBuilderUtils.buildTextField(
        ...
        if (isLimit) ...[
          ...
          PropertyBuilderUtils.buildTextField(
            ...
            onChanged: (val) =>
                _emit(policy.copyWith(submissionLimitCount: int.tryParse(val))),
          ),
          const SizedBox(height: 12),
