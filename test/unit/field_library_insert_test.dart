...
Future<ProviderContainer> _makeContainer() async {
  ...
  // Wait until the async build finishes.
  await container.read(formBuilderControllerProvider(key).future);
  return container;
}
...
void main() {
  group('FormBuilderController — addQuestionToActiveSection', () {
    test('inserts a question into the first section when none is selected', () async {
      ...
      final stateBefore = container.read(formBuilderControllerProvider(_key)).value!;
      ...
      final stateAfter = container.read(formBuilderControllerProvider(_key)).value!;
      expect(stateAfter.form.sections[0].questions, hasLength(1));
      expect(stateAfter.form.sections[0].questions[0].type, QuestionType.mobile);
    ...
    test('inserts into the explicitly selected section (not always the first)', () async {
      final container = await _makeContainer();
      addTearDown(container.dispose);
      ...
      final stateWithTwo =
          container.read(formBuilderControllerProvider(_key)).value!;
      expect(stateWithTwo.form.sections, hasLength(2));
      final secondId = stateWithTwo.form.sections[1].id;
      expect(stateWithTwo.selectedSectionId, secondId); // addSection selects new section
      ...
      expect(stateAfter.form.sections[1].questions[0].type, QuestionType.email);
    });
  ...
  group('FormBuilderController — addTemplateToActiveSection', () {
    ...
    test('a section-type template appends a new section regardless of selection', () async {
      final container = await _makeContainer();
      addTearDown(container.dispose);
  ...
  group('FormBuilderController — addQuestion (direct, section-specific)', () {
    test('click insert is distinct from selection-only: state has both new question and selection', () async {
      final container = await _makeContainer();
      addTearDown(container.dispose);
      ...
      final state = container.read(formBuilderControllerProvider(_key)).value!;
      ...
      expect(state.selectedQuestionId, state.form.sections[0].questions[0].id);
